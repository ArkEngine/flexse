...
//        )
//{
//    uint32_t finish_term_num = 0;    ///< ÓÐ¶àÉÙ¸öterm±éÀúÍêÁË
//    int current_pointer_list[terminfo_size];   ///< Ã¿¸ötermµÄÀ­Á´£¬µ±Ç°Ö¸ÏòµÄÎ»ÖÃ
...
//        printf("min : %u\n", min_id);
//
//        ///< °ÑµÈÓÚ×îÐ¡idµÄ£¬Ëã³öËüµÄweight£¬È»ºó°Ñ¶ÔÓ¦µÄÖ¸ÕëÍùÏÂÒÆ¶¯
//        uint32_t hit = 0;
//        uint32_t weight = 0;
//
//        for(uint32_t i=0; i<terminfo_size; i++)
...
//                    current_offset ++;     ///< Ö¸ÕëÏòÏÂÒÆ¶¯Ò»¸ö
//                    if(current_offset == (int32_t)terminfo_list[i].list_size)  ///< À­Á´±éÀúÍêÁË
//                    {
//                        printf("[%d] over.\n", i);
//                        current_offset = -1;
//                        finish_term_num ++;                    
...
//            }
//        }
//        if (hit == terminfo_size) // ËùÓÐµÄÊý×éÀï¶¼³öÏÖÁË¡£
//        {
...
//
//
//    ///< 2. ¶ÔËùÓÐ¿ÉÄÜµÄ½á¹û¾¡ÐÄÅÅÐò£¬µÃµ½×î¶àMAX_RESULT_NUM¸ö½á¹û
//    int32_t result_num = 0;
//    if(result_pair_list.size() <= result_list_size)   ///< Õû¸övectorÍêÕûÅÅÐòÒ»±é
//    {
